...
authors <- vapply(
    Filter(x = params$meta, function(x) identical(x$name, "author")), 
    `[[`, character(1), "content"
)
...
for (i in seq_len(nrow(params$content))) {
    img <- tag(
        "div",
        attr = list(class = "slide-image"), 
        tag("img", attr = list(
            src = params$content$slide_path[i],
            alt = paste("Slide", params$content$id_slide[i], "preview")
        ))
    )
    slide_content <- if (params$content$content[i] != "") {
        tag("div", attr = list(class = "slide-content"), params$content$content[i])
    }
    slide_notes <- if (params$content$notes[i] != "") {
        tag("div", attr = list(class = "slide-notes"), params$content$notes[i])
    }
