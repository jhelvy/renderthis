...
Some output types depend on intermediate outputs. Here is a diagram of the render hierarchy:
...
All `to_*()` functions use the `from` and `to` arguments.
...
**Note**: Rendering the PDF requires a [local installation of Google Chrome](#local-chrome-installation-requirement)
